...
class TestRouterCore:
    ...
    def test_route_pick_none_when_all_weekly_walled(self):
        rows = [accounts_row("gmail", 0.0, seven_day=100.0),
                accounts_row("acme", 0.0, seven_day=100.0)]
        assert accounts.route_pick(rows, set()) is None
...
class TestCmdPickEnv:
    ...
    def test_fable_mode_uses_flat_row_shape(self, monkeypatch, capsys):
        ...
        self._wire(monkeypatch, blobs, {"gmail": self._tok("gmail"), "alumni": self._tok("alumni")},
                   resets=resets, mode="fable")
        accounts.cmd_pick_env(types.SimpleNamespace())
        out = capsys.readouterr().out
        # gmail is best 5h but fable-heavy; alumni has real fable headroom -> wins
        assert "export CLAUDE_CODE_OAUTH_TOKEN='sk-ant-alumni'" in out
